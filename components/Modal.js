...
export default function Modal({ isVisible, onClose, setIsConnected }) {
  const { activate } = useWeb3React();

  const hanldeMetamaskConnection = async () => {
    try {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-30 backdrop-blur-[2px] flex justify-center items-center">
      <div className="w-[320px] h-[200px] md:w-[320px] sm:w-[320px] lg:w-[320px] m-2 bg-[#535353] rounded-[30px] p-[1px]">
        <div className="h-full bg-[#0c0e1b] text-white rounded-[30px]">
          {/* modal header */}
          <div className="header border-b-[0.1rem] border-[#2e335c] bg-gradient-to-r from-[#252743] to-[#0d0f1f] rounded-t-[30px]">
            <div className="flex p-4">
              ...
              <button
                ...
                onClick={() => onClose()}
              >
                <img src="/assets/close-btn.png" className="h-5 w-auto" />
              </button>
            </div>
          </div>
          ...
          <div className="w-full py-2 px-4">
            <div className="flex flex-wrap w-full h-full my-3">
              <div className="w-1/2 text-center">
                <button onClick={hanldeMetamaskConnection}>
                  <svg
                    viewBox="0 0 40 40"
                    width="40px"
                    color="text"
                  ...
                  <span className="text-center">Metamask</span>
                </button>
              </div>
              <div className="w-1/2 text-center">
                <button onClick={handleWalletConnection}>
                  <img
                    src="/assets/wallet-connect.c0072cb0.svg"
                    className="w-[40px] h-[40px] mx-auto"
                  />
                  <span className="text-center">Binance Wallet</span>
                </button>
              </div>
            </div>
          </div>
